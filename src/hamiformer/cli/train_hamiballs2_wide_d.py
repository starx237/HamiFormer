...
from hamiformer.baselines.physiformer import HamiBalls2WideD, parameter_count
...
def model_forward(model: HamiBalls2WideD, batch: dict[str, torch.Tensor], noisy: torch.Tensor, tau: torch.Tensor) -> torch.Tensor:
...
@torch.no_grad()
def validate(model: HamiBalls2WideD, loader: DataLoader, *, device: torch.device, scale: torch.Tensor, validation_seed: int) -> dict[str, Any]:
    model.eval()
...
def main() -> None:
    ...
    if metadata.get('semantic_hash') != dataset_signature(cfg):
        raise RuntimeError('dataset semantic hash differs from configuration')
    ...
    model = HamiBalls2WideD(**cfg['model']).to(device)
    ...
    if checkpoint is not None:
        if 'train_rf_generator_state' in checkpoint:
            train_generator.set_state(checkpoint['train_rf_generator_state'])
        if 'train_order_generator_state' in checkpoint:
            shuffle_generator.set_state(checkpoint['train_order_generator_state'])
    ...
    (output / 'run_contract.json').write_text(json.dumps({'config': cfg, 'parameters': params, 'seeds': seed_record}, indent=2), encoding='utf-8')
    ...
    while step < target_steps:
        for raw_batch in train_loader:
            ...
            running_count += 1
            if step % 100 == 0:
                print(json.dumps({'step': step, 'loss': running_loss / running_count, 'lr': lr}), flush=True)
